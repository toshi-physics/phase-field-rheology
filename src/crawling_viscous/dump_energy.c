...
void energyOutput(EnergyDump* dump, PhaseFieldModel* model, int step) {
  ...
  for (int n = 0; n < model->numOfCells; n++) {
	...
	gradx = grad2(i, j, iu, id, 0, cellField);
	grady = grad2(i, j, ju, jd, 1, cellField);
	cahnHilliard += 0.25*model->cahnHilliardCoeff*phi2*dphi2;
	surfaceTension += 0.5*model->surfaceTensionCoeff*
	...
	repulsion += 0.5*model->repulsionCoeff*(phi2Sum*phi2Sum-
						dump->phi4Field[x][y]);
      }
    }
    dV = 1.0-cellVolume/model->piR2phi02;
    volumeConst += model->volumePenaltyCoeff*dV*dV;
  }
  //  energy = cahnHilliard + volumeConst + repulsion;
  energy = cahnHilliard + surfaceTension + volumeConst + repulsion;
  fprintf(f, "%d %.5f %.5f %.5f %.5f %.5f\n", step, cahnHilliard, 
	  surfaceTension, volumeConst, repulsion, energy);
  fclose(f);
}
