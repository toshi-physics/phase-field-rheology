...
# Get the artist for plotting additional polygons
pc = PolyCollection([], color="grey", edgecolor="black", linewidth=2.0)
plt_addpoly = ax.add_collection(pc)
...
#        pt = tuple(vor.points[r])
    ...
    # Add polygon for the dragged bead
    bead_region = vor.regions[vor.point_region[0]]
    bead_poly = np.stack([vor.vertices[i] for i in bead_region])
    plt_addpoly.set_paths([bead_poly,])

    # Plot the time label
    if (use_label):
        plt_time_txt.set_text(r"$D_rt = {:.1f}$".format(time_map[frame]*Dr*dt))
    
    return plt_pts, plt_polygons, plt_addpoly, 
